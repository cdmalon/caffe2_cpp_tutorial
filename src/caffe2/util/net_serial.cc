...
std::string net_short_op(const OperatorDef& def) {
  ...
  if (def.has_device_option() && def.device_option().has_device_id()) {
    stream << " cuda_gpu_id:" << def.device_option().device_id();
  }
...
std::string net_short_net(const NetDef& def) {
  ...
  if (def.has_device_option() && def.device_option().has_device_id()) {
    stream << "cuda_gpu_id:" << def.device_option().device_id() << std::endl;
  }
